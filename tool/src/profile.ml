...
let () =
  try
    print_endline "]] HumIDIFy: version 1.0 ,-.";
    print_endline "]------------------------|-'";
    if Array.length Sys.argv = 3 then
      ...
      printf "-> File      : %s\n-> Profile   : %s (with confidence %.2f%%)\n\n" binary label (confidence *. 100.);

      let profile_path = "include/" ^ label ^ ".bfdl" in

      let profile = Lang.load_profile Lang.default_environment profile_path in
      match Lang.evaluate_profile ~ida_path profile label (shs, plt_bound, sym_strs, img, syms, imps, strs) binary with
        | None -> invalid_arg "failed to evaluate profile against executable"
        | Some b -> printf "-> Judgement : %s\n" (if not b then "potentially anomalous" else "likely benign")
    else
